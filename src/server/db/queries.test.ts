...
import { createWorkItemCommand, listMemoWorkItems, listWorkItems, updateWorkItemStatusCommand } from "@/server/db/queries/work-items";
...
describe("SQLite work item and classification queries", () => {
  ...
  it("quick captures a memo and classifies it into a work item", async () => {
    const ctx = setup();
    ...
    const itemId = await classifyMemoCommand({
      workspaceId: ctx.workspaceId,
      userId: ctx.userId,
      memoId,
      repositoryId: null,
    ...
    expect((await listMemoWorkItems(ctx.workspaceId))[0]).toMatchObject({ id: memoId, status: "itemized" });
    expect((await listWorkItems(ctx.workspaceId)).map((item) => item.id)).toContain(itemId);
  });
});
