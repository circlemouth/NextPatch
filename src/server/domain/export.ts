...
import {
  getLocalExportContext,
  insertExportLog,
  readWorkspaceExportEntities,
  type ExportFormat
} from "@/server/db/queries/export";
...
export async function createBackupDocument(format: ExportFormat = "json"): Promise<BackupDocument> {
  const { user, workspace } = getLocalExportContext();
  const entities = readWorkspaceExportEntities(workspace.id);
