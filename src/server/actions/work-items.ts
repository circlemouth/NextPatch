...
import { createWorkItemRecord, updateWorkItemStatusRecord } from "@/server/db/queries/work-items";
...
export async function createWorkItem(formData: FormData) {
  ...
  const id = await createWorkItemRecord({
...
export async function updateWorkItemStatus(formData: FormData) {
  ...
  await updateWorkItemStatusRecord(workspace.id, user.id, id, status);
