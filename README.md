...
NextPatch は、リポジトリごとのメモ・タスク管理、バグ、実装メモ、ChatGPT の貼り付けメモ、次に取るべきアクションを整理するための、自己管理型のローカルサーバー Web アプリです。
...
`/login` は LAN 内利用向けの共通パスワードログイン画面です。ログインに成功すると署名付き HttpOnly Cookie セッションを発行し、指定された安全な内部パス、または既定のホームである `/repositories` に遷移します。ログアウトは右上メニューの「ログアウト」から行います。
